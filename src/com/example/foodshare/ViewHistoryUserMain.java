...
public class ViewHistoryUserMain extends Activity {
	String uname,fname,quantity,details,pickuplocation,pickupendtime,expiretime,cdate,phone;
	SQLiteDatabase db;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_view_history_user_main);
		...
		Intent it=getIntent();
		 ...
		 db=openOrCreateDatabase("foodconservation", MODE_PRIVATE, null);
			Cursor c=db.rawQuery("select * from addfooduser where phone='"+phone+"' and fname='"+fname+"'", null);
			try{
			c.moveToFirst();
			if(c!=null){
				do {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.activity_view_history_user_main, menu);
		return true;
	}
