...
public class ViewHistoryRestMain extends Activity {
String rno,fname,quantity,details,pickuplocation,pickupendtime,expiretime,cdate;
SQLiteDatabase db;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_view_history_rest_main);
		...
		Intent it=getIntent();
		 ...
		 db=openOrCreateDatabase("foodconservation", MODE_PRIVATE, null);
			Cursor c=db.rawQuery("select * from addfoodres where rno='"+rno+"' and fname='"+fname+"'", null);
			try{
			c.moveToFirst();
			if(c!=null){
				do {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.activity_view_history_rest_main, menu);
		return true;
	}
