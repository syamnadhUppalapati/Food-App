	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bt.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				Cursor c=db.rawQuery("select * from userreg where uname='"+username+"' and phone='"+password+"'", null);
				try{
				c.moveToFirst();
				if(c!=null){
					do {
						int i=c.getColumnIndex("uname");
						 uname=c.getString(i);
						int j=c.getColumnIndex("phone");
						 pwd=c.getString(j);
						 if(username.equalsIgnoreCase(uname)&&(password.equalsIgnoreCase(pwd))){
								Intent it=new Intent(UserLogin.this,UserHome.class);
								it.putExtra("uname", uname);
								it.putExtra("phone", pwd);
								startActivity(it);
							}
